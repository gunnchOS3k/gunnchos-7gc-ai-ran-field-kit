...
Gate~3 evidence derives from phone-first Edge-IO probes validated against \texttt{contracts/edge\_measurement\_batch.v1.schema.json}.
Each sample records ISO-8601 \texttt{timestamp}, optional \texttt{latency\_ms}, \texttt{quality\_flags} (e.g., \texttt{probe\_timeout}), optional explicit \texttt{service\_available}, workload and service profile identifiers, and \texttt{local\_edge\_response\_ms} when probed.
Probes are sorted temporally; consecutive timestamps define observation intervals.
Unavailable classification follows the precedence in Section~\ref{sec:system_model}.
The metric engine (\texttt{scripts/compute\_recovery\_time.py}) constructs outage events, computes \texttt{total\_service\_outage\_time\_s}, emits per-event \texttt{time\_to\_recovery\_s} records, flags right censoring, and reports probe cadence statistics (median interval, missing-probe gaps $>$2.5$\times$ median).
Interval-censoring uncertainty is bounded by $\pm$ half the median probe interval---true transition times may fall between probes.
Model fields such as \texttt{expected\_recovery\_time\_s} in resilience bundles are never merged into physical outcomes.
...
Ethics documentation does not imply regulatory certification or law-enforcement monitoring capabilities.
