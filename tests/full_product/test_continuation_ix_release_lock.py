...
def test_cont_ix_artifacts_exist_and_honest():
    ...
    assert lock["digital_release_lock_complete"] is False
    assert lock["ready_for_npi_dfm_and_evt_quotation"] is False
    assert lock["final_umbrella"] is False
    assert lock["physical_execution_freeze"] is True
    assert lock["recommendation"] == "CONTINUE_DIGITAL_RELEASE_ENGINEERING"
    assert lock["accepted_mains"]["gunnchos-7gc-ai-ran-field-kit"].startswith("7c6b955")
    assert lock["accepted_mains"]["gunnchos-device-os"].startswith("06366da")
    assert lock["accepted_mains"]["gunnchos-hardware-industrial-design"].startswith("a710f35")
    ...
    assert burndown["digital_release_lock_complete"] is False
    assert burndown["recommendation"] == "CONTINUE_DIGITAL_RELEASE_ENGINEERING"
    ...
    assert matrix["digital_release_lock_complete"] is False
    ...
    assert drafts["digital_release_lock_complete"] is False
