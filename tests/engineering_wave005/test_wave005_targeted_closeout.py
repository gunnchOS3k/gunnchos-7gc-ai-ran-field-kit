"""Validate Wave 005 targeted closeout artifact and register freeze."""
...
def test_targeted_row_diff_freeze():
    ...
    diff_path = root / "artifacts/engineering_wave005_closeout/TARGETED_ROW_DIFF.json"
    ...
    assert data["closeout_validation"]["ENGINEERING_WAVE_005_TARGETED_CLOSEOUT_VALIDATION_PASS"] is True
    assert data["claim_boundaries"]["CARRIER_ACCEPTED"] is False
    assert data["claim_boundaries"]["STANDARDIZED_6G"] is False
    assert data["claim_boundaries"]["PRODUCTION_APP_PRIORITY_SIGNING"] is False
    assert data["claim_boundaries"]["FIELD_MEASURED_PERFORMANCE"] is False
    ...
    assert x["complete_delta"] == 12
    assert x["impl_open_delta"] == 12
...
def test_closeout_result_and_queues():
    root = Path(__file__).resolve().parents[2]
    result = json.loads(
        (root / "artifacts/engineering_wave005_closeout/CLOSEOUT_RESULT.json").read_text(encoding="utf-8")
    )
    reg = json.loads(
    ...
    totals = reg["totals"]
    assert result["ENGINEERING_WAVE_005_TARGETED_CLOSEOUT_VALIDATION_PASS"] is True
    assert result["ENGINEERING_WAVE_005_CLOSEOUT"] == "COMPLETE"
