...
def main() -> int:
    ...
    if errors:
        ...
        for e in errors:
            print("-", e)
        return 1
    # Artifacts OK when C1–C5/C7 complete; C6 may remain IN_PROGRESS (product-vision residual).
    crit = st.get("criteria") or {}
    required = [f"G2-C{i}" for i in range(1, 8)]
    missing = [k for k in required if k not in crit]
    if missing:
        print("GATE2_NONPHYSICAL_INCOMPLETE")
        for m in missing:
            print("- missing criterion", m)
        return 1
    incomplete = {k: v for k, v in crit.items() if v != "NONPHYSICAL_COMPLETE"}
    if set(incomplete) <= {"G2-C6"} and incomplete.get("G2-C6") == "IN_PROGRESS":
        print("GATE2_NONPHYSICAL_ARTIFACTS_OK")
        print("gate2_nonphysical_status=IN_PROGRESS")
        print("residual=G2-C6_PRODUCT_VISION_RUNTIME")
        print("claim_allowed=GATE_2_NONPHYSICAL_IN_PROGRESS")
