# Phase XIII — Frontier Parity Campaign Plan

**Status:** PLAN ONLY (not execution)  
**Scope amendment:** Option **4** (full campaign) **+** Option **2** (mandatory Wave 0 = Phase XII X1 close) — **unchanged**  
**Doctrine:** `PHYSICAL_EXECUTION_FREEZE=ACTIVE` · DRAFT PRs only · Cursor never merges · never claim `GUNNCHOS_FRONTIER_OS_PARITY` or `GUNNCHAI_FRONTIER_PRODUCT_PARITY` without Part Q / AI gate evidence · historical FULL_* tokens remain historical/internal  

**Workspace:** `/Users/gunnchos/Downloads/gunnchos-7gc-research-product-spine/repos`

**Integration layer:** [COMPETITIVE_GAP_MASTER_REGISTER.md](./COMPETITIVE_GAP_MASTER_REGISTER.md) binds OS + AI frontier requirements to Phase XII / FULL PRODUCT DoD. Machine namespaces: `FOS-*` (OS), `FAI-*` (AI), `FEC-*` (ecosystem cross-product).

**Current token honesty** (`TOKENS.json`):
...
| `FULL_GUNNCHAI3K_PLATFORM_DIGITAL_COMPLETE` | Historical/internal only; prior AI requirement scope |
| `GUNNCHOS_FRONTIER_OS_PARITY` | Reject until all Part Q OS gates are non-`INCOMPLETE_DIGITAL` with prove artifacts — remains **false** |
| `GUNNCHAI_FRONTIER_PRODUCT_PARITY` | Reject until all Part P AI gates are non-`INCOMPLETE_DIGITAL` with prove artifacts — remains **false** |
| `GUNNCHOS3K_FRONTIER_ECOSYSTEM_PARITY` | Requires OS + AI frontier parity + real journey parity + physical validation for claimed HW — remains **false** |
...
| Layer | Path |
|---|---|
| Field-kit control plane (canonical) | `program/frontier_parity/` |
| Legacy OS-only plan mirror | `program/frontier_os_parity/` (superseded paths → prefer `frontier_parity/`) |
| Master register | `program/frontier_parity/COMPETITIVE_GAP_MASTER_REGISTER.md` |
| OS / AI requirements (normative MD) | `program/frontier_parity/gunnchOS_*` / `gunnchAI_*` |
| Machine requirements | `program/frontier_parity/requirements/{FOS,FAI,FEC}_requirements.yaml` |
| Field-kit artifacts mirror | `artifacts/phase_xiii/` |
| Frontier claim firewall | `program/claims/phase_xiii_frontier_parity_firewall.yaml` |
...
| Owner mirrors | `gunnchos-device-os/requirements/frontier/`, `gunnchAI3k/requirements/frontier/` |
| Branches | `phase-xiii/frontier-os-parity` (+ wave / FAI suffixes) |
...
Do **not** bury under `program/full_product/continuation_*` — Continuations V–IX stay digital-release totality; Frontier is Phase XIII. Historical `requirements.yaml` counts stay unchanged; frontier IDs live only in the new FOS/FAI/FEC namespace.
...
flowchart TD
  ...
  OSParity[GUNNCHOS_FRONTIER_OS_PARITY_false]
  AIW1[FAI_Wave_Model_Fleet]
  AIW2[FAI_Wave_Memory_Projects]
  AIW3[FAI_Wave_Search_Agents]
  AIW4[FAI_Wave_MM_Voice_Skills]
  AIParity[GUNNCHAI_FRONTIER_PRODUCT_PARITY_false]
  FEC[FEC_coordination]
  Eco[GUNNCHOS3K_FRONTIER_ECOSYSTEM_PARITY_false]
  W0 --> W1 --> W2 --> W3 --> W4 --> W5 --> W6 --> W7 --> W8 --> W9 --> W10 --> W11 --> W12 --> W13 --> OSParity
  W0 --> AIW1 --> AIW2 --> AIW3 --> AIW4 --> AIParity
  W1 -.-> FEC
  AIW1 -.-> FEC
  OSParity --> Eco
  AIParity --> Eco
...
**AI frontier (FAI) waves run in parallel after Wave 0**, coordinated via `FEC-*` ecosystem requirements and the master register — they do not wait for Wave 13 OS completion, but Wave 8 (OS AI primitive) and FAI waves must stay FEC-aligned.
...
**Deliverables under `program/frontier_parity/`:**

| Artifact | Purpose |
|---|---|
| `COMPETITIVE_GAP_MASTER_REGISTER.md` | OS↔AI integration layer / DoD binder |
| `gunnchOS_FRONTIER_OS_PARITY_REQUIREMENTS.md` | Normative OS Parts A–U |
| `gunnchAI_FRONTIER_PRODUCT_PARITY_REQUIREMENTS.md` | Normative AI Parts A–U |
| `requirements/{FOS,FAI,FEC}_requirements.yaml` | Machine IDs (new namespace; historical counts unchanged) |
| `REQUIREMENT_OWNERS.yaml` / `REQUIREMENT_OWNERSHIP.yaml` | Every FOS/FAI/FEC ID → owner repo + role |
| `PARITY_GATES.yaml` | All Part Q / AI gates with status enum |
...
| `TOKENS.json` | All frontier tokens false |

**Firewall:** `program/claims/phase_xiii_frontier_parity_firewall.yaml`

```yaml
# Conceptual rules (to implement)
- reject GUNNCHOS_FRONTIER_OS_PARITY unless all OS gates != INCOMPLETE_DIGITAL
- reject GUNNCHAI_FRONTIER_PRODUCT_PARITY unless all AI gates != INCOMPLETE_DIGITAL
- reject GUNNCHOS3K_FRONTIER_ECOSYSTEM_PARITY unless OS+AI+journey+physical conditions met
...
**Parity earn rule:** `GUNNCHOS_FRONTIER_OS_PARITY` may become TRUE only when zero OS gates are `INCOMPLETE_DIGITAL` and firewall + prove suite green. Same rule for `GUNNCHAI_FRONTIER_PRODUCT_PARITY` against AI gates. Conditional/physical/external gates must be explicitly classified — never hidden. Both tokens remain **false** until earned; ecosystem token stays **false** until both plus journey/physical conditions in the master register.
...
| S-FAI* | FAI waves (parallel post-W0) | gunnchAI3k primary, device-os AI interface, field-kit FEC/gates |

FAI parallel track (after Wave 0; FEC-coordinated with OS waves):

| FAI wave | Focus | Primary DRAFT PRs |
|---|---|---|
| FAI-1 | Multi-model fleet / router | gunnchAI3k, field-kit |
| FAI-2 | Memory + Projects | gunnchAI3k, field-kit |
| FAI-3 | Search/Research + Agents | gunnchAI3k, device-os, field-kit |
| FAI-4 | Multimodal/voice/skills/artifacts/evals | gunnchAI3k, field-kit |
...
```
APPROVE_PLAN → EXECUTE_WAVE_0_PHASE_XII_X1_CLOSE → THEN_WAVE_1_SCAFFOLD
  → OS WAVES_2_TO_13 || FAI WAVES (parallel, FEC-coordinated)
GUNNCHOS_FRONTIER_OS_PARITY = false / NOT_CLAIMABLE
GUNNCHAI_FRONTIER_PRODUCT_PARITY = false / NOT_CLAIMABLE
GUNNCHOS3K_FRONTIER_ECOSYSTEM_PARITY = false / NOT_CLAIMABLE
